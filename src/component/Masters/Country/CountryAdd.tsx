...
const schema = Joi.object({
  ...
  country_short_name: Joi.string().min(3).max(30).required().messages({
    'string.empty': 'Username is required',
    'string.min': 'Username should have at least 3 characters',
    'string.max': 'Username should have at most 30 characters',
  }),
  country_phone_code: Joi.string().min(6).required().messages({
...
const CountryAdd = () => {
  ...
  //     const user = userlist?.find((e) => e.id === updateHistory?.id);
      ...
      <Container component="main" maxWidth="xs">
        ...
        >
          ...
          <Typography component="h1" variant="h5">
            {updateHistory && updateHistory.currentType === 'add' ? 'Add new Country' : 'Edit Country Details'}
          </Typography>
          <Box component="form" noValidate onSubmit={handleSubmit(handleRegister)} sx={{ mt: 3 }}>
            <Grid container spacing={2}>
              ...
              {errors.country_short_name && errors.country_short_name?.message && (
                <span className="error">{errors.country_short_name?.message}</span>
              )}
              <Grid item xs={12}>
                <TextField
                  required
              ...
              {errors.country_phone_code && errors.country_phone_code?.message && (
                <span className="error">{errors.country_phone_code?.message}</span>
              )}
            </Grid>
            ...
            <Grid container>
              <Grid item xs>
                <Button
                  ...
                  sx={{ mt: 3, mb: 2 }}
                >
                  {updateHistory && updateHistory.currentType === 'add' ? 'Add Country' : 'Submit'}
                </Button>
              </Grid>
              <Grid item>
                <Button
